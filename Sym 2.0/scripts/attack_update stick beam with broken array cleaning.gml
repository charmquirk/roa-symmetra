            	...
            	if attack_down && ammo > 0 {		// Aiming Mode. Lets you move the beam.
					...
					with oPlayer {				// Scanning players
						if (player != other.player && get_player_team(player) != get_player_team(other.player)) {	// If not me and not on my team
							if collision_line(other.x - other.spr_dir*other.arm_offset_x + round(other.beam_offset*dcos(other.weapon_aim)), other.y - other.arm_offset_y - round(other.beam_offset*dsin(other.weapon_aim)), other.beam_x, other.beam_y, hurtboxID, false, false) {	// If I (enemy player) have my hurtbox collide with the beam line.
								var b_dist = point_distance(x, y - char_height/2, other.x - other.spr_dir*other.arm_offset_x + round(other.beam_offset*dcos(other.weapon_aim)), other.y - other.arm_offset_y - round(other.beam_offset*dsin(other.weapon_aim)));
								if array_find_index(other.beam_list, id) == -1 {
									array_push(other.beam_list, id);		// Add me to the list of people the beam can hit		[b_dist]
								}
							}
						}
            		}
            		if array_length(beam_list) > 0 {	// If there are eligible targets
            			for (i = 0; i < array_length(beam_list); i++;) {	// Descend through list of eligible targets
            				if beam_list[i] == noone {			// If slot empty
            					...
            					if array_length(beam_list) > 1  {
            						...
            						beam_list[i] = beam_list[i+1];		// Move next value down to fill current empty index
            						if array_find_index(beam_list, beam_list[i+1]) == array_length(beam_list) -1 { // If just moved down last value
            							array_slice(beam_list,0,array_find_index(beam_list, beam_list[i]));		// Shorten array
            							break;
            						}
            					}
            					// Sorts through the array pushing values forward until it reaches the end and starts shortening the array
            				}
            				else {
            					if collision_line(x - spr_dir*arm_offset_x + round(beam_offset*dcos(weapon_aim)), y - arm_offset_y - round(beam_offset*dsin(weapon_aim)), beam_x, beam_y, beam_list[i].hurtboxID, false, false) {
            		...
            		if array_length(beam_target) > 1 {
            			for (i = 0; i < array_length(beam_target); i++;) {
            				beam_target[i] = beam_target;
            			}
            		}
            		if beam_target != noone && collision_line(x - spr_dir*arm_offset_x + round(beam_offset*dcos(weapon_aim)), y - arm_offset_y - round(beam_offset*dsin(weapon_aim)), beam_x, beam_y, beam_target.hurtboxID, false, false) {
            			if beam_target.object_index = oPlayer && beam_target.invincible == true || beam_target.state == PS_AIR_DODGE || beam_target.state == PS_SPAWN || beam_target.state == PS_DEAD || beam_target.state == PS_RESPAWN || beam_target.state == PS_ROLL_BACKWARD || beam_target.state == PS_ROLL_FORWARD || beam_target.state == PS_WALL_TECH { //If target should be unable to hit
            				beam_target = noone;
            				beam_chip = 0;
            			}
            		...
            		// 			}
            					if (charge < charge_max) {		// If not at max charge
            						charge += min(charge_max - charge, 1);	// Charge beam
            		...
            		// 		}
    						...
    						if beam_chip >= beam_chip_threshold && beam_target != noone {
					    		spawn_hit_fx(beam_target.x, beam_target.y - beam_target.char_height/2, cfx_beam_fire);
        						take_damage(beam_target.player, id.player, 1);
        						beam_chip = 0;
